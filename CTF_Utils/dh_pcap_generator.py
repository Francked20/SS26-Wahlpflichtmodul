#!/usr/bin/env python3
"""Offline alternative to the day-3 weak-DH (Logjam-style) challenge's
live VM capture (see dh_export_vm_listener.py): writes a single .pcap
...
(GET /dh_export/vm_replay_data only returns server-role hex), so this
reads the dh_export_variants collection directly via pymongo - same
...
Usage:
    python3 dh_pcap_generator.py --index 42 --out capture.pcap
"""
import argparse
import os
import random
...
def fetch_variant(index: int) -> dict:
    ...
    variant = client[MONGO_DB].dh_export_variants.find_one({"index": index})
    if variant is None:
        raise SystemExit(f"No dh_export_variants doc with index={index}")
    return variant
...
def main():
    ...
    parser.add_argument("--index", type=int, required=True,
                         help="weak-DH variant index (0-99) to embed as real TLS traffic")
