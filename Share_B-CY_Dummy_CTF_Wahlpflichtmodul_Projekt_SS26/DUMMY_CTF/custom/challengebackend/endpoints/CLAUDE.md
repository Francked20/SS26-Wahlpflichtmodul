...
Each module (except `export_cipher.py`) is a standalone crypto challenge with
its own hardcoded key/flag list, indexed by `{index}` in the URL (one variant
per team/player to prevent flag sharing) — no shared code between modules.
