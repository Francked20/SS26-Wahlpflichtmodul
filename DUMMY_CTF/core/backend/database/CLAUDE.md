...
- **models.py** — all Document/embedded models:
  ...
  - `Challenge` — the static definition of a task (day/task id, points, question
    ...
    simultaneous solves. Optional `dynamic_check` field covers two independent
    dynamic-verification subsystems, dispatched by prefix in `check_answer()`:
    `"export_*"` (day-3 export-cipher/FREAK challenge) routes to
    `utils/export_cipher_client.check_dynamic_answer()`; `"dh_*"` (day-4
    weak-Diffie-Hellman/Logjam challenge) routes to
    `utils/dh_export_client.check_dynamic_answer()`. Either way `check_answer()`
    skips the normal `solutions[]` comparison and makes an HTTP call to
    `custom/challengebackend`, which owns the actual variant data/verification
    logic for both — then falls through into the same
