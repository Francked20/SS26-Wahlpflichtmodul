...
- **export_cipher_client.py** — thin httpx client for the day-3 export-cipher
  ...
  `check_dynamic_answer()` when `dynamic_check` starts with `"export_"`;
  `endpoints/challenges.py`'s `/solve` calls `get_reveal_factor()` after a
  correct `export_factor256` answer). All the actual crypto/variant-pool logic
  lives in `custom/challengebackend` now, reached over the internal `ctf-net`
  network (`http://challenge:8000`, `X-Challenge-Api-Key` header) — same
...
- **dh_export_client.py** — mirrors `export_cipher_client.py` exactly, one
  level up: same fail-closed httpx-over-`ctf-net` pattern, but for the day-4
  weak-Diffie-Hellman (Logjam-style) challenge (`dynamic_check` values
  prefixed `"dh_"`). Only exposes `check_dynamic_answer()` — this chain has no
  reveal-on-solve reward, so no `get_reveal_factor()` equivalent.
