...
TLS_CLIENT_IP, TLS_SERVER_IP = "10.0.1.10", "10.0.1.20"
TLS_CLIENT_PORT, TLS_SERVER_PORT = 52000, 443
...
def fetch_variants():
    ...
    return list(client[MONGO_DB].export_cipher_variants.find({}))
...
def tcp_conversation(src_ip, dst_ip, sport, dport, flights):
    ...
    def segment(src, dst, sport_, dport_, flags, seq, ack, payload=b""):
        pkt = (Ether(src="02:00:00:00:01:01", dst="02:00:00:00:01:02") /
...
def decoy_http():
    ...
    return tcp_conversation("10.0.1.30", "10.0.1.40", 53000, 80, [(True, req), (False, resp)])


def decoy_dns():
    eth = dict(src="02:00:00:00:01:01", dst="02:00:00:00:01:02")
    query = (Ether(**eth) / IP(src="10.0.1.30", dst="10.0.1.1") / UDP(sport=54000, dport=53) /
             Raw(bytes.fromhex("bbbb01000001000000000000") + b"\x07example\x03com\x00\x00\x01\x00\x01"))
    response = (Ether(**eth) / IP(src="10.0.1.1", dst="10.0.1.30") / UDP(sport=53, dport=54000) /
                Raw(bytes.fromhex("bbbb81800001000100000000") +
...
def main():
    ...
    if not variants:
        raise SystemExit(
            "No export_cipher_variants found - run "
            "custom/challengebackend/scripts/generate_export_cipher_pool.py first."
        )
    for v in variants:
        index = v["index"]
