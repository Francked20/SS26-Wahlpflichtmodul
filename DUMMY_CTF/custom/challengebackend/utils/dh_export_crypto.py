"""Crypto und TLS 1.0 Wire-Format-Helfer fuer die weak-DH (Logjam) Challenge"""
...
# weak DH parameters + ServerKeyExchange/ClientKeyExchange

def gen_weak_dh_params(total_bits: int, factor_bits: int, rng: random.Random,
                       max_tries: int = 200000):
    """Weak prime p = 2q + 1 mit q = Produkt kleiner Primzahlen (p-1 smooth)"""
...
def _rsa_sign_md5sha1(n: int, d: int, data: bytes) -> bytes:
    """RSA-Signatur: MD5+SHA1-Hash, PKCS#1 v1.5 type-01 Padding, dann RSA"""
...
def craft_dh_variant(p: int, g: int, factors: list, n_rsa: int, d_rsa: int,
    ...
    """Kompletter TLS 1.0 DHE_EXPORT-Handshake (4 flights) mit weak DH params"""
